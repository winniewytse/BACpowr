...
cubature::cuhre(
  function(arg) {
    x <- arg[1]
    y <- arg[2]
    pow_crt2(J = 20, n = 50, delta = x, rho = y,
...
## Sample Size Determination (`Jn_crt2()`)
...
```{r}
loss <- function(J) {
  ep_crt2(J = J, n = 50, delta = .5, delta_sd = .2, rho = .3,
...
```{r}
loss <- function(n) {
  ep_crt2(J = 65, n = n, delta = .5, delta_sd = .2, rho = .3,
          rho_sd = .1, rsq2 = 0, K = 0, P = .5, power = .8,
          alpha = .05, test = "two.sided") - .8
}
