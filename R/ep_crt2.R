...
#' @param minEval Minimum number of evaluations of the integral. Defaults to
#'    `50` to stabilize results.
#' @param ... Additional arguments to be passed to \code{cubuture::cuhre()} to
...
#' ep_crt2(J = 30, n = 100, delta = .5, delta_sd = .2, rho = .1, rho_sd = .05,
#'         rsq2 = .3)
ep_crt2 <- function(J, n, delta, delta_sd, rho, rho_sd, rsq2 = 0, K = 0, P = .5,
                    power = .8, alpha = .05, test = "two.sided", minEval = 50,
                    ...) {
  ...
  if (delta_sd == 0) {
    if (rho_sd == 0) { # (1) delta_sd = rho_sd = 0
      ...
      cubature::cuhre(
        function(x) {
          do.call(pow_crt2, append(list(rho = x, delta = delta), params)) *
    ...
    if (rho_sd == 0) { # (3) rho_sd = 0
      cubature::cuhre(
        function(x) {
          do.call(pow_crt2, append(list(rho = rho, delta = x), params)) *
      ...
      cubature::cuhre(
        function(arg) {
          x <- arg[1]; y <- arg[2]
          do.call(pow_crt2, append(list(rho = y, delta = x), params)) *
