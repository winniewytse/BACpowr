...
#'   \eqn{\omega = \frac{\tau_1^2}{\tau_0^2}} where \eqn{\tau_0^2} is the variance
#'   of the intercept random component and \eqn{\tau_1^2} is the variance of the
#'   treatment random effect.
#' @param omega_sd Uncertainty level of the treatment effect heterogeneity estimate.
#' @param rsq1 Estimate of variance explained by the level-1 (e.g., the
...
#' @return Assurance level given a two-level CRT design with J clusters that have
#'  n observations each.
#' @export
#' @examples
#' al_msrt2(J = 30, n = 100, delta = .5, delta_sd = .2, rho = .1, rho_sd = .05,
#'          omega = 0.5, omega_sd = 0.2)
  ...
  if (delta_sd == 0) {
    if (rho_sd == 0) {
      if (omega_sd == 0) { # (1) delta_sd = rho_sd = omega_sd = 0
        do.call(pow_msrt2,
                append(list(omega = omega, rho = rho, delta = delta), params))
      } else { # (2) delta_sd = rho_sd = 0
        omega_ab <- gamma_ab(omega, omega_sd)
        stats::pgamma(
          do.call(inv_pow_msrt2,
                  append(list(delta = delta, rho = rho, power = power), params)),
      ...
      if (omega_sd == 0) { # (3) delta_sd = omega_sd = 0
        ...
        # boundary checking
        lo_bound <- do.call(inv_pow_msrt2,
                            append(list(delta = delta, omega = 0, power = power),
                                   params))
        up_bound <- do.call(inv_pow_msrt2,
                            append(list(delta = delta, omega = 1, power = power),
                                   params))
        ...
        cubature::cuhre(
          function(x) {
            stats::pbeta(
              do.call(inv_pow_msrt2,
                      append(list(delta = delta, omega = x, power = power),
                             params)),
              shape1 = rho_ab[1], shape2 = rho_ab[2], lower.tail = lower.tail
    ...
    if (test == "two.sided") {
      if (rho_sd == 0) {
        if (omega_sd == 0) { # (5) rho_sd = omega_sd = 0
          d_star <- do.call(inv_pow_msrt2,
                            append(list(rho = rho, omega = omega, power = power),
                                   params))
          ...
          cubature::cuhre(
            function(x) {
              d_star <- do.call(inv_pow_msrt2, append(list(rho = rho, omega = x,
                                                           power = power), params))
              (stats::pnorm(d_star, mean = delta, sd = delta_sd, lower.tail = FALSE) +
                  stats::pnorm(-d_star, mean = delta, sd = delta_sd, lower.tail = TRUE)) *
                stats::dgamma(x, shape = omega_ab[1], rate = omega_ab[2])
        ...
        if (omega_sd == 0) { # (7) omega_sd = 0
          ...
          cubature::hcubature(
            function(x) {
              d_star <- do.call(inv_pow_msrt2,
                                append(list(rho = x, omega = omega,
                                            power = power), params))
              (stats::pnorm(d_star, mean = delta, sd = delta_sd,
                            lower.tail = FALSE) +
                  stats::pnorm(-d_star, mean = delta, sd = delta_sd,
                               lower.tail = TRUE)) *
      ...
      if (rho_sd == 0) {
        if (omega_sd == 0) { # (5) rho_sd = omega_sd = 0
          d_star <-do.call(inv_pow_msrt2,
                           append(list(delta = delta, rho = rho, omega = omega,
                                       power = power), params))
          stats::pnorm(d_star, mean = delta, sd = delta_sd, lower.tail = FALSE)
        } else { # (6) rho_sd = 0
          omega_ab <- gamma_ab(omega, omega_sd)
        ...
        if (omega_sd == 0) { # (7) omega_sd = 0
          ...
          cubature::cuhre(
            function(y) {
              d_star <- do.call(inv_pow_msrt2,
                                append(list(rho = y, delta = delta,
                                            omega = omega, power = power),
                                       params))
              stats::pnorm(d_star, mean = delta, sd = delta_sd,
                           lower.tail = FALSE) *
